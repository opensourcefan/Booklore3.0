...
public class KoreaderUserController {
    ...
    @Operation(summary = "Toggle sync progress with Fable reader", description = "Enable or disable syncing reading progress with Fable reader.")
    @ApiResponse(responseCode = "204", description = "Sync progress toggled successfully")
    @PatchMapping("/me/sync-progress-with-fable")
    @PreAuthorize("@securityUtil.canSyncKoReader() or @securityUtil.isAdmin()")
    public ResponseEntity<Void> toggleSyncProgressWithFable(
            @Parameter(description = "Enable or disable sync progress") @RequestParam boolean enabled) {
        koreaderUserService.toggleSyncProgressWithFable(enabled);
        return ResponseEntity.noContent().build();
    }
}
