...
import org.fable.model.dto.request.*;
...
import org.fable.service.metadata.MetadataManagementService;
...
public class MetadataController {
    ...
    public ResponseEntity<BookMetadata> updateMetadata(
        ...
        org.fable.model.dto.Book book = bookMetadataService.updateMetadata(bookId, metadataUpdateWrapper, mergeCategories, replaceMode);
    ...
    public ResponseEntity<BookMetadata> wipeMetadata(@Parameter(description = "ID of the book") @PathVariable long bookId) {
        org.fable.model.dto.Book wipedBook = bookMetadataService.wipeBookMetadata(bookId);
    ...
    public ResponseEntity<Integer> restoreTitlesFromFilenames(@Parameter(description = "List of book IDs") @RequestBody BulkBookIdsRequest request) {
        List<org.fable.model.dto.Book> restoredBooks = bookMetadataService.restoreTitlesFromFilename(request.getBookIds());
    ...
    public ResponseEntity<List<BookMetadata>> toggleAllMetadata(@Parameter(description = "Toggle all lock request") @RequestBody ToggleAllLockRequest request) {
        List<org.fable.model.dto.Book> books = bookMetadataService.toggleAllLock(request);
        List<BookMetadata> metadataList = books.stream().map(org.fable.model.dto.Book::getMetadata).collect(java.util.stream.Collectors.toList());
    ...
    public ResponseEntity<List<BookMetadata>> toggleFieldLocks(@Parameter(description = "Toggle field locks request") @RequestBody ToggleFieldLocksRequest request) {
        List<org.fable.model.dto.Book> books = bookMetadataService.toggleFieldLocks(request.getBookIds(), request.getFieldActions());
        List<BookMetadata> metadataList = books.stream().map(org.fable.model.dto.Book::getMetadata).collect(java.util.stream.Collectors.toList());
        return ResponseEntity.ok(metadataList);
    }
