package org.fable.service.progress;
...
import org.fable.model.dto.progress.KoboProgress;
import org.fable.model.dto.progress.KoProgress;
import org.fable.model.dto.progress.PdfProgress;
import org.fable.model.dto.request.BookFileProgress;
...
public class ReadingProgressService {
    ...
    public void updateReadProgress(ReadProgressRequest request) {
        ...
        FableUserEntity userEntity = findUserOrThrow(user.getId());
    ...
    private void saveToUserBookFileProgress(FableUserEntity user, BookFileProgress fileProgress, Instant now) {
    ...
    private void saveToUserBookFileProgressFromLegacy(FableUserEntity user, BookFileEntity bookFile,
    ...
    private void performReset(Long userId, Set<Long> bookIds, ResetProgressType type, Instant now) {
        ...
        switch (type) {
            case FABLE -> {
                userBookProgressRepository.bulkResetFableProgress(userId, bookIdList, now);
    ...
    private void validateResetPermission(List<Long> bookIds, FableUser user, ResetProgressType type) {
        if (bookIds.size() <= 1) return;
        UserPermission permission = switch (type) {
            case FABLE -> UserPermission.CAN_BULK_RESET_FABLE_READ_PROGRESS;
    ...
    private FableUserEntity findUserOrThrow(Long userId) {
