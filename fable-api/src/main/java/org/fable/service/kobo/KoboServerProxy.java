...
import org.fable.util.kobo.FableSyncTokenGenerator;
...
public class KoboServerProxy {
    ...
    private final FableSyncTokenGenerator fableSyncTokenGenerator;
    ...
    public ResponseEntity<JsonNode> proxyCurrentRequest(Object body, boolean includeSyncToken) {
        ...
        FableSyncToken syncToken = null;
        if (includeSyncToken) {
            syncToken = fableSyncTokenGenerator.fromRequestHeaders(request);
    ...
    private ResponseEntity<JsonNode> executeProxyRequest(HttpServletRequest request, Object body, String path, boolean includeSyncToken, FableSyncToken syncToken) {
        try {
            ...
            if (responseHeaders.getFirst(KoboHeaders.X_KOBO_SYNCTOKEN) != null && includeSyncToken && syncToken != null) {
                ...
                if (koboToken != null) {
                    FableSyncToken updated = FableSyncToken.builder()
                            ...
                            .build();
                    responseHeaders.set(KoboHeaders.X_KOBO_SYNCTOKEN, fableSyncTokenGenerator.toBase64(updated));
                }
            }
