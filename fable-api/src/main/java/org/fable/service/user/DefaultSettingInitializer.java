...
public class DefaultSettingInitializer {
    ...
    @Transactional
    public void ensureDefaultSettings(FableUser fableUser) {
        if (initializedUsers.contains(fableUser.getId())) {
            return;
        }
        synchronized (("user-init-" + fableUser.getId()).intern()) {
            if (initializedUsers.contains(fableUser.getId())) return;
            FableUserEntity user = userRepository.findById(fableUser.getId()).orElseThrow(() -> new UsernameNotFoundException("User not found"));
            ...
            initializedUsers.add(fableUser.getId());
        }
    }

    private void addSettingIfMissing(FableUserEntity user, UserSettingKey key, Object value) {
    ...
    private void patchPerBookSetting(FableUserEntity user) {
        user.getSettings()
                .stream()
                ...
                .ifPresent(setting -> {
                    try {
                        var current = objectMapper.readValue(setting.getSettingValue(), FableUser.UserSettings.PerBookSetting.class);
                        if (current.getCbx() == null) {
                            current.setCbx(FableUser.UserSettings.PerBookSetting.GlobalOrIndividual.Individual);
