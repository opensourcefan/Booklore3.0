...
public class PdfMetadataExtractor implements FileMetadataExtractor {
                ...
                try (InputStream is = metadata.createInputStream()) {
                    if (is == null) {
                        ...
                        if (StringUtils.isNotBlank(rawXmp)) {
                            ...
                            extractFableMetadata(xpath, doc, metadataBuilder);
    ...
    private void extractFableMetadata(XPath xpath, Document doc, BookMetadata.BookMetadataBuilder builder) {
        try {
            // Series information (now in Fable namespace, not Calibre)
            String seriesName = extractFableField(xpath, doc, "seriesName");
            if (StringUtils.isNotBlank(seriesName)) {
                builder.seriesName(seriesName);
            }

            String seriesNumber = extractFableField(xpath, doc, "seriesNumber");
            ...
            String seriesTotal = extractFableField(xpath, doc, "seriesTotal");
            ...
            String subtitle = extractFableField(xpath, doc, "subtitle");
            log.debug("Extracted subtitle (camelCase): '{}'", subtitle);
            if (StringUtils.isBlank(subtitle)) {
                subtitle = xpath.evaluate("//fable:Subtitle/text()", doc);
            ...
            // ISBNs from Fable namespace
            String isbn13 = extractFableField(xpath, doc, "isbn13");
            if (StringUtils.isNotBlank(isbn13)) {
                builder.isbn13(isbn13.trim());
            }
            
            String isbn10 = extractFableField(xpath, doc, "isbn10");
            if (StringUtils.isNotBlank(isbn10)) {
                builder.isbn10(isbn10.trim());
            }

            // External IDs from Fable namespace
            String googleId = extractFableField(xpath, doc, "googleId");
            if (StringUtils.isNotBlank(googleId)) {
                builder.googleId(googleId.trim());
            }

            String goodreadsId = extractFableField(xpath, doc, "goodreadsId");
            if (StringUtils.isNotBlank(goodreadsId)) {
                builder.goodreadsId(goodreadsId.trim());
            }

            String hardcoverId = extractFableField(xpath, doc, "hardcoverId");
            if (StringUtils.isNotBlank(hardcoverId)) {
                builder.hardcoverId(hardcoverId.trim());
            }

            String hardcoverBookId = extractFableField(xpath, doc, "hardcoverBookId");
            if (StringUtils.isNotBlank(hardcoverBookId)) {
                builder.hardcoverBookId(hardcoverBookId.trim());
            }

            String asin = extractFableField(xpath, doc, "asin");
            if (StringUtils.isNotBlank(asin)) {
                builder.asin(asin.trim());
            }

            String comicvineId = extractFableField(xpath, doc, "comicvineId");
            if (StringUtils.isNotBlank(comicvineId)) {
                builder.comicvineId(comicvineId.trim());
            }

            String lubimyczytacId = extractFableField(xpath, doc, "lubimyczytacId");
            if (StringUtils.isNotBlank(lubimyczytacId)) {
                builder.lubimyczytacId(lubimyczytacId.trim());
            }

            String ranobedbId = extractFableField(xpath, doc, "ranobedbId");
            ...
            Set<String> moods = extractFableBag(xpath, doc, "moods");
            log.debug("Extracted moods from RDF Bag: {}", moods);
            if (moods.isEmpty()) {
                // Legacy format support
                String moodsLegacy = xpath.evaluate("//fable:Moods/text()", doc);
            ...
            Set<String> tags = extractFableBag(xpath, doc, "tags");
            log.debug("Extracted tags from RDF Bag: {}", tags);
            if (tags.isEmpty()) {
                // Legacy format support
                String tagsLegacy = xpath.evaluate("//fable:Tags/text()", doc);
            ...
            extractFableRating(xpath, doc, "amazonRating", "AmazonRating", builder::amazonRating);
            extractFableRating(xpath, doc, "goodreadsRating", "GoodreadsRating", builder::goodreadsRating);
            extractFableRating(xpath, doc, "hardcoverRating", "HardcoverRating", builder::hardcoverRating);
            extractFableRating(xpath, doc, "lubimyczytacRating", "LubimyczytacRating", builder::lubimyczytacRating);
            extractFableRating(xpath, doc, "ranobedbRating", "RanobedbRating", builder::ranobedbRating);
            
            // User rating
            extractFableRating(xpath, doc, "rating", "Rating", builder::rating);

        } catch (Exception e) {
            log.warn("Failed to extract fable metadata: {}", e.getMessage(), e);
        }
    }

    /**
     * Extracts a simple text field from Fable namespace.
     */
    private String extractFableField(XPath xpath, Document doc, String fieldName) {
        try {
            return xpath.evaluate("//fable:" + fieldName + "/text()", doc);
        } catch (Exception e) {
            return "";
        }
    }
    ...
    /**
     * Extracts an RDF Bag as a Set of strings from Fable namespace.
     */
    private Set<String> extractFableBag(XPath xpath, Document doc, String fieldName) {
        Set<String> values = new HashSet<>();
        try {
            String xpathExpr = "//fable:" + fieldName + "/rdf:Bag/rdf:li/text()";
            ...
            log.warn("Failed to extract RDF Bag for fable:{}: {}", fieldName, e.getMessage());
        }
        return values;
    }
     ...
     */
    private void extractFableRating(XPath xpath, Document doc, String newName, String legacyName, 
                                        java.util.function.Consumer<Double> setter) {
        try {
            String value = xpath.evaluate("//fable:" + newName + "/text()", doc);
            if (StringUtils.isBlank(value)) {
                value = xpath.evaluate("//fable:" + legacyName + "/text()", doc);
            }
    ...
    private static class XmpNamespaceContext implements NamespaceContext {
        ...
        public XmpNamespaceContext() {
            ...
            prefixMap.put("fable", "http://fable.org/metadata/1.0/");
        }
