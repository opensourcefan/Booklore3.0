...
class UserProvisioningServiceTest {
    ...
    @Captor
    private ArgumentCaptor<FableUserEntity> userCaptor;
    ...
    void setUp() {
        when(userRepository.save(any())).thenAnswer(invocation -> {
            var user = invocation.getArgument(0, FableUserEntity.class);
            user.setId(1L);
            return user;
        });
    ...
    void provisionOidcUser_setsBasicFieldsCorrectly() {
        FableUserEntity result = userProvisioningService.provisionOidcUser(
                "jdoe", "jdoe@example.com", "John Doe",
                "sub-123", "https://issuer.example.com", "https://avatar.example.com/jdoe.png",
                provisionDetails);

        verify(userRepository).save(userCaptor.capture());
        FableUserEntity saved = userCaptor.getValue();
