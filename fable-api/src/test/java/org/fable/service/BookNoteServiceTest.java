...
import org.fable.service.book.BookNoteService;
...
class BookNoteServiceTest {
    ...
    void createOrUpdateNote_updatesExistingNote_whenIdIsPresent() {
        ...
        when(userRepository.findById(userId)).thenReturn(Optional.of(FableUserEntity.builder().id(userId).isDefaultPassword(false).build()));
