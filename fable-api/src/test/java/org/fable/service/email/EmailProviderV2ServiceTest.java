...
class EmailProviderV2ServiceTest {
    ...
    private FableUser regularUser;
    private EmailProviderV2Entity savedEntity;
    private EmailProviderV2 providerDto;

    @BeforeEach
    void setUp() {
        FableUser.UserPermissions adminPerms = new FableUser.UserPermissions();
        adminPerms.setAdmin(true);
        adminUser = FableUser.builder().id(1L).username("admin").permissions(adminPerms).build();

        FableUser.UserPermissions regularPerms = new FableUser.UserPermissions();
        regularPerms.setAdmin(false);
        regularUser = FableUser.builder().id(2L).username("user").permissions(regularPerms).build();
