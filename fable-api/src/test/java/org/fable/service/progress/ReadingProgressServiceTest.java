...
class ReadingProgressServiceTest {
    ...
    @Test
    void resetProgress_fable_shouldCallBulkReset() {
        FableUser user = mock(FableUser.class);
        when(authenticationService.getAuthenticatedUser()).thenReturn(user);
        when(user.getId()).thenReturn(1L);

        FableUser.UserPermissions permissions = mock(FableUser.UserPermissions.class);
        when(user.getPermissions()).thenReturn(permissions);
        when(permissions.isCanBulkResetFableReadProgress()).thenReturn(true);
        ...
        List<BookStatusUpdateResponse> result = readingProgressService.resetProgress(bookIds, ResetProgressType.FABLE);

        verify(userBookProgressRepository).bulkResetFableProgress(eq(1L), eq(new ArrayList<>(bookIds)), any());
        assertEquals(2, result.size());
    }
    ...
    void resetProgress_shouldAllowSingleBookWithoutPermission() {
        ...
        FableUser.UserPermissions permissions = mock(FableUser.UserPermissions.class);
        when(user.getPermissions()).thenReturn(permissions);
        when(permissions.isCanBulkResetFableReadProgress()).thenReturn(false);
        ...
        assertDoesNotThrow(() -> readingProgressService.resetProgress(bookIds, ResetProgressType.FABLE));
    }
    ...
    void resetProgress_shouldThrowIfNoBulkPermission() {
        ...
        assertThrows(APIException.class, () -> readingProgressService.resetProgress(bookIds, ResetProgressType.FABLE));
    }
